...
contract StorageTokenContractTwo is ReentrancyGuard, Ownable {
    ...
    constructor() {
        // MAFIA
        mafia = IERC20(0xC2f27D6874137cB4582dB791373630ECFa77c5d8);
        // USDT
        rewardsToken = husdtTokenAddress;

        _uniswapV2Router = IPancakeRouter02(pancakeRouterAddress);
    }

    function addRewardsTokenForU(uint256 tokenAmount) public {
        mafia.safeTransferFrom(_msgSender(), address(this), tokenAmount);

        //token -> usdt
        address[] memory path = new address[](2);
        path[0] = address(mafia);
        path[1] = address(rewardsToken);

        mafia.safeApprove(pancakeRouterAddress, tokenAmount);

        // token:address(this) token -> address(uniswapV2Pair)
        // usdt:address(uniswapV2Pair) token -> address(sender)
