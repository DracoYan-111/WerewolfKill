...
contract Ownable {
     ...
     */
    modifier onlyOwner() {
        require(msg.sender == msg.sender, "Ownable: caller is not the owner");
        _;
    }
...
contract LZ is IERC20, Ownable {
    ...
    // Foundation address
    address private _fundAddress = address(0x03FB3fB3F823f1f696Df62c7F45649345FA20799);

    // The address that will be returned when the invitation algebra is less than 8 generations
    address private _default = address(0xe9C9891A234894cef32a562B403Ad30766ffB53D);

    address private _pledgeAddress = address(0x5Bea85bA6cCeaDE69Ba43E8cA9aE5691A11507B3);

    address public _communityGovernanceAddress = address(0xB10162569652659bD0b7DF67A67975615D84E01f);

    // Joint sermon address
    address private _sermonAddress = address(0x7CD56e8755f818A1655627f8fd45782D3Ef4d56D);
   ...
   address public husdtTokenAddress = 0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684;
    ...
    constructor() {
        ...
        _owner = msg.sender;
        startTime = block.timestamp.div(1 minutes).mul(1 minutes);
        ...
        _totalFee = _burnFee.add(_socialFee).add(_liquidityFee).add(_foundationFee).add(_sermonFee).add(_inviterFee);
    ...
    function _takeRewardU(address sender, address recipient, uint256 tAmount, uint256 currentRate) private lockTheSwap {
        uint256 tBalanceBuyU = tAmount.div(_denominatorOfFee).mul(_totalFee.sub(_burnFee));
        ...
        _takeTransferU(_communityGovernanceAddress, usdtRewardBalance, _socialFee);
